...
Type::Type(const Location& loc, TokenType modifier, bool isRef, bool isSimd /*= false*/)
    : Node(loc) 
    , modifier_(modifier)
    , isRef_(isRef)
    , simd_(isSimd)
...
MemberFctInfo Type::hasMemberFct(const std::string& id, const TypeList& in, Module* m) const
{
    return hasMemberFct(&id, in, m);
}
...
const llvm::Type* ErrorType::getRawLLVMType(Module* m, int simdLength /*= 0*/) const
{
    swiftAssert(false, "unreachable");
    return 0;
}
...
MemberFctInfo ErrorType::hasMemberFct(const std::string* id, const TypeList& in, Module* m) const
{
    swiftAssert(false, "unreachable");
    return MemberFctInfo(MemberFctInfo::FALSE);
}
...
BaseType::BaseType(const Location& loc, TokenType modifier, std::string* id, bool isInOut, bool isSimd /*= false*/)
...
MemberFctInfo BaseType::hasMemberFct(const std::string* id, const TypeList& in, Module* m) const
{
    Class* c = lookupClass(m);
    swiftAssert(c, "must be found");
    MemberFct* fct = c->lookupMemberFct(m, id, in);

    if (fct)
    {
        bool simd = fct->isSimd();

        if ( fct->isAutoGenerated() )
            return MemberFctInfo(MemberFctInfo::COPY, simd, fct);
        else
            return MemberFctInfo(MemberFctInfo::USER, simd, fct);
    }
    else
        return MemberFctInfo(MemberFctInfo::FALSE);
}
...
ScalarType::ScalarType(const Location& loc, TokenType modifier, std::string* id, bool isSimd /*= false*/)
...
const llvm::Type* ScalarType::getRawLLVMType(Module* m, int simdLength /*= 0*/) const
{
    const llvm::Type* llvmType = (*typeMap_)[ *id() ];

    if (simdLength)
        return llvm::VectorType::get(llvmType, simdLength);
    else
        return llvmType;
}
...
UserType::UserType(const Location& loc, TokenType modifier, std::string* id, bool isInOut /*= false*/, bool isSimd /*= false*/)
...
const llvm::Type* UserType::getRawLLVMType(Module* m, int simdLength /*= 0*/) const
{
    ...
    if (simdLength)
    {
        swiftAssert( simdLength == c->getSimdLength(), "simd lengths must match" );
        return c->getVecType();
    }
    else
        return c->getLLVMType();
}
...
NestedType::NestedType(const Location& loc, TokenType modifier, bool isRef, Type* innerType)
...
Ptr::Ptr(const Location& loc, TokenType modifier, Type* innerType)
...
const llvm::Type* Ptr::getRawLLVMType(Module* m, int simdLength /*= 0*/) const
{
...
MemberFctInfo Ptr::hasMemberFct(const std::string* id, const TypeList& in, Module* m) const
{
    // ptr only suports a copy create and assign
    if ( (*id == "=" || *id == "create") && in.size() == 1 && check(in[0], m) )
        return MemberFctInfo(MemberFctInfo::COPY);
    else
        return MemberFctInfo(MemberFctInfo::USER);
}

//------------------------------------------------------------------------------

Container::Container(const Location& loc, TokenType modifier, Type* innerType)
    : NestedType(loc, modifier, false, innerType)
{}
...
MemberFctInfo Container::hasMemberFct(const std::string* id, const TypeList& in, Module* m) const
{
    if (in.size() == 1 && *id == "create")
    {
        if ( in[0]->isIndex() )
            return MemberFctInfo(MemberFctInfo::CONTAINER_CREATE);
        else if ( check(in[0], m) )
            return MemberFctInfo(MemberFctInfo::CONTAINER_COPY);
    }
...
Array::Array(const Location& loc, TokenType modifier, Type* innerType)
...
const llvm::Type* Array::getRawLLVMType(Module* m, int simdLength /*= 0*/) const
{
...
Simd::Simd(const Location& loc, TokenType modifier, Type* innerType)
    : Container(loc, modifier, innerType)
{}
...
const llvm::Type* Simd::getRawLLVMType(Module* m, int simdLength /*= 0*/) const
{
    swiftAssert(simdLength == 0, "todo");
