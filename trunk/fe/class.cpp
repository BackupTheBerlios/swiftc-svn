...
{
    if ( const Create* create = dynamic_cast<const Create*>(this) )
        return create->isAutoCopy() || create->isAutoDefault();
    else if ( const Assign* assign = dynamic_cast<const Assign*>(this) )
        return assign->isAutoCopy();
