...
spill_reload
    ...
    {
        EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg()))
    }
    | X64_RELOAD any_type X64_REG_1
    {
        EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg()))
    }
    ;
...
load_store
    ...
    { 
        EMIT(mnemonic("mov", $2) << '\t' << memvar2str($3, $1->getOffset()) << ", " << reg2str($1->resReg())) 
    }
    | X64_LOAD any_type any_reg
    { 
        EMIT(mnemonic("mov", $2) << '\t' << ptr2str($3, $1->getOffset()) << ", " << reg2str($1->resReg()))
    }
    | X64_LOAD any_type X64_MEM_VAR any_reg
    { 
        EMIT(mnemonic("mov", $2) << '\t' << memvar_index2str($3, $4, $1->getOffset()) << ", " << reg2str($1->resReg())) 
    }
    | X64_LOAD any_type any_reg any_reg
    { 
        EMIT(mnemonic("mov", $2) << '\t' << ptr_index2str($3, $4, $1->getOffset()) << ", " << reg2str($1->resReg()))
    }
...
int_mov
    ...
    { 
        EMIT(mnemonic("mov", $2) << '\t' << cst2str($3) << ", " << reg2str($1->resReg())) 
    }
...
int_add_and
    : add_or_and int_type X64_CONST X64_CONST /* mov (c1 + c2), r1 */
    {
        EMIT(mnemonic("mov", $2) << '\t' << cst_op_cst($1, $3, $4) << ", " << reg2str($1->resReg())) 
    } 
    | add_or_and int_type X64_CONST X64_REG_1 /* add c, r1 */
    {
        EMIT(mnemonic(instr2str($1), $2) << '\t' << cst2str($3) << ", " << reg2str($4)) 
    }
    | add_or_and int_type X64_CONST X64_REG_2 /* mov c, r1; add r2, r1 */ 
    {
        EMIT(mnemonic("mov", $2) << '\t' << cst2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($4) << ", " << reg2str($1->resReg())) 
    }
    | add_or_and int_type X64_REG_1 X64_CONST /* add c, r1 */
    {
        EMIT(mnemonic(instr2str($1), $2) << '\t' << cst2str($4) << ", " << reg2str($3)) 
    }
    | add_or_and int_type X64_REG_1 X64_REG_1 /* shl 2, r2 */
    { 
        if ($1->kind_ == '+')
            EMIT(mnemonic("shl", $2) << " $2, " << reg2str($3))
        /* else -> do nothing */
    }
    | add_or_and int_type X64_REG_1 X64_REG_2 /* add r2, r1 */
    { 
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($4) << ", " << reg2str($3)) 
    }
    | add_or_and int_type X64_REG_2 X64_CONST /* mov c, r1; add r2, r1 */ 
    { 
        EMIT(mnemonic("mov", $2) << '\t' << cst2str($4) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg())) 
    }
    | add_or_and int_type X64_REG_2 X64_REG_1 /* add r2, r1 */
    { 
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($3) << ", " << reg2str($4)) 
    }
    | add_or_and int_type X64_REG_2 X64_REG_2 /* mov r2, r1; shl 2, r1 */
    { 
        EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg()))
        if ($1->kind_ == '+')
            EMIT(mnemonic("shl", $2) << " $2,  " << reg2str($1->resReg())) 
        /* else -> do nothing */
    }
    | add_or_and int_type X64_REG_2 X64_REG_3 /* mov r2, r1; add r3, r1 */
    { 
        EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($4) << ", " << reg2str($1->resReg())) 
    }
...
int_mul
    ...
    {
        EMIT(mnemonic("imul", $2) << '\t' << cst2str($3) << ", " << reg2str($4)) 
    }
    | X64_MUL int_type X64_CONST X64_REG_2 /* mov c, r1; mul r2, r1 */ 
    {
        EMIT(mnemonic("mov", $2) << '\t' << cst2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic("imul", $2) << '\t' << reg2str($4) << ", " << reg2str($1->resReg())) 
    }
    ...
    { 
        EMIT(mnemonic("mov", $2) << '\t' << cst2str($4) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic("imul", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg())) 
    }
    | X64_MUL int_type X64_REG_2 X64_REG_1 /* mul r2, r1 */
    { 
        EMIT(mnemonic("imul", $2) << '\t' << reg2str($3) << ", " << reg2str($4)) 
    }
...
int_sub
    ...
    { 
          EMIT(mnemonic("sub", $2) << '\t' << cst2str($3) << ", " << reg2str($4)) 
          EMIT(mnemonic("neg", $2) << '\t' << reg2str($4)) 
    }
    | X64_SUB int_type X64_CONST X64_REG_2 /* mov c, r1; sub r2, r1 */
    { 
          EMIT(mnemonic("mov", $2) << '\t' << cst2str($3) << ", " << reg2str($1->resReg()))
          EMIT(mnemonic("sub", $2) << '\t' << reg2str($4) << ", " << reg2str($1->resReg())) 
    }
    ...
    { 
          EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg()))
          EMIT(mnemonic("sub", $2) << '\t' << cst2str($4) << ", " << reg2str($1->resReg())) 
    }
    | X64_SUB int_type X64_REG_2 X64_REG_1 /* sub r2, r1; neg r1*/
    { 
          EMIT(mnemonic("sub", $2) << '\t' << reg2str($3) << ", " << reg2str($4))
          EMIT(mnemonic("neg", $2) << '\t' << reg2str($4)) 
    }
    | X64_SUB int_type X64_REG_2 X64_REG_2 /* xor r1, r1 */
    { 
          EMIT(mnemonic("xor", $2) << '\t' << reg2str($1->resReg()) << ", " << reg2str($1->resReg())) 
    }
...
sint_un_minus
    ...
    { 
          EMIT(mnemonic("mov", $2) << '\t' << un_minus_cst($1, $3) << ", " << reg2str($1->resReg())) 
    } 
    | X64_UN_MINUS sint_type X64_REG_1 /* xor neg_mask, r1 */
    { 
          EMIT(mnemonic("xor", $2) << '\t' << neg_mask($2) << ", " << reg2str($1->resReg())) 
    }
...
sint_no8_div
    ...
    { 
          EMIT(mnemonic("mov", $2) << '\t' << cst_op_cst($1, $3, $4, true) << ", " << reg2str($1->resReg())) 
    }
    | X64_DIV sint_no8_type X64_CONST X64_REG_2 /* mov sgn_cst(c), rdx; mov c, r1; idiv r2 */
    { 
          EMIT(mnemonic("mov", $2) << '\t' << sgn_cst2str($3) << ", " << rdx2str(($2)))
          EMIT(mnemonic("mov", $2) << '\t' << cst2str($3) << ", " << reg2str($1->resReg())) 
          EMIT(mnemonic("idiv", $2) << '\t' << reg2str($4))
    }
    ...
    { 
          ...
          EMIT(mnemonic("idiv", $2) << '\t' << mcst2str($4)) 
    }
    ...
    { 
          EMIT(mnemonic("imov", $2) << " $1, " << reg2str($1->resReg()))
    }
...
uint_no8_div
    ...
    { 
          ...
          EMIT(mnemonic("mov", $2) << '\t' << cst2str($3) << ", " << reg2str($1->resReg())) 
          EMIT(mnemonic("div", $2) << '\t' << reg2str($4))
    }
    | X64_DIV uint_no8_type X64_REG_1 X64_CONST /* xor rdx, rdx; div c */
    { 
          EMIT(mnemonic("xor", $2) << '\t' << rdx2str($2) << ", " << rdx2str($2))
          EMIT(mnemonic("div", $2) << '\t' << mcst2str($4)) 
    }
    ...
    { 
          EMIT(mnemonic("mov", $2) << " $1, " << reg2str($1->resReg()))
    }
...
real_simd_mov
    : X64_MOV real_simd_type X64_UNDEF 
    { 
        /* emit no code: mov X64_UNDEF, %r1 */ 
    }
    | X64_MOV real_simd_type X64_CONST 
    { 
        EMIT(mnemonic("mov", $2) << '\t' << mcst2str($3) << ", " << reg2str($1->resReg())) 
    }
    | X64_MOV real_simd_type X64_REG_1 
    { 
        /* emit no code: mov %r1, %r1 */ 
    }
    | X64_MOV real_simd_type X64_REG_2 
    { 
        EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg())) 
    }
    ;
...
real_simd_add_mul
    : add_or_mul real_simd_type X64_CONST X64_CONST /* mov (c1 + c2), r1 */
    { 
        EMIT(mnemonic("mov", $2) << '\t' << cst_op_cst($1, $3, $4, true) << ", " << reg2str($1->resReg())) 
    }
    | add_or_mul real_simd_type X64_CONST X64_REG_1 /* add c, r1 */
    { 
        EMIT(mnemonic(instr2str($1), $2) << '\t' << mcst2str($3) << ", " << reg2str($4)) 
    }
    | add_or_mul real_simd_type X64_CONST X64_REG_2 /* mov c, r1; add r2, r1 */ 
    { 
        EMIT(mnemonic("mov", $2) << '\t' << mcst2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($4) << ", " << reg2str($1->resReg())) 
    }
    | add_or_mul real_simd_type X64_REG_1 X64_CONST /* add c, r1 */
    { 
        EMIT(mnemonic(instr2str($1), $2) << '\t' << mcst2str($4) << ", " << reg2str($3)) 
    }
    | add_or_mul real_simd_type X64_REG_1 X64_REG_1 /* add r1, r1 */
    { 
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($3) << ", " << reg2str($3)) 
    }
    | add_or_mul real_simd_type X64_REG_1 X64_REG_2 /* add r2, r1 */
    { 
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($4) << ", " << reg2str($3)) 
    }
    | add_or_mul real_simd_type X64_REG_2 X64_CONST /* mov c, r1; add r2, r1 */ 
    { 
        EMIT(mnemonic("mov", $2) << '\t' << mcst2str($4) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg())) 
    }
    | add_or_mul real_simd_type X64_REG_2 X64_REG_1 /* add r2, r1 */
    { 
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($3) << ", " << reg2str($4)) 
    }
    | add_or_mul real_simd_type X64_REG_2 X64_REG_2 /* mov r2, r1; add r1, r1 */
    { 
        EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($1->resReg()) << ", " << reg2str($1->resReg())) 
    }
    | add_or_mul real_simd_type X64_REG_2 X64_REG_3 /* mov r2, r1; add r3, r1 */
    { 
        EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic(instr2str($1), $2) << '\t' << reg2str($4) << ", " << reg2str($1->resReg())) 
    }
...
real_simd_sub
    : X64_SUB real_simd_type X64_CONST X64_CONST /* mov (c1 - c2), r1 */
    {
        EMIT(mnemonic("mov", $2) << '\t' << cst_op_cst($1, $3, $4, true) << ", " << reg2str($1->resReg())) 
    }
    | X64_SUB real_simd_type X64_CONST X64_REG_1 /* sub c, r1; xor signmask, r1 */
    {
        EMIT(mnemonic(instr2str($1), $2) << '\t' << mcst2str($3) << ", " << reg2str($4))  
        EMIT(mnemonic("xor", $2) << '\t' << neg_mask($2, true) << ", " << reg2str($4)) 
    }
    | X64_SUB real_simd_type X64_CONST X64_REG_2 /* mov c, r1; sub r2, r1 */ 
    {
        EMIT(mnemonic("mov", $2) << '\t' << mcst2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic("sub", $2) << '\t' << reg2str($4) << ", " << reg2str($1->resReg())) 
    }
    | X64_SUB real_simd_type X64_REG_1 X64_CONST /* sub c, r1 */
    {
        EMIT(mnemonic("sub", $2) << '\t' << mcst2str($4) << ", " << reg2str($3)) 
    }
    | X64_SUB real_simd_type X64_REG_1 X64_REG_1 /* xor r1, r1 */
    {
        EMIT(mnemonic("xor", $2) << '\t' << reg2str($3) << ", " << reg2str($3)) 
    }
    | X64_SUB real_simd_type X64_REG_1 X64_REG_2 /* sub r2, r1 */
    {
        EMIT(mnemonic("sub", $2) << '\t' << reg2str($4) << ", " << reg2str($3)) 
    }
    | X64_SUB real_simd_type X64_REG_2 X64_CONST /* mov r2, r1; sub c, r1 */ 
    {
        EMIT(mnemonic("mov", $2) << '\t' <<  reg2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic("sub", $2) << '\t' << mcst2str($4) << ", " << reg2str($1->resReg())) 
    }
    | X64_SUB real_simd_type X64_REG_2 X64_REG_1 /* sub r2, r1 */
    {
        EMIT(mnemonic("sub", $2) << '\t' << reg2str($3) << ", " << reg2str($4)) 
    }
    | X64_SUB real_simd_type X64_REG_2 X64_REG_2 /* xor r1, r1 */
    {
        EMIT(mnemonic("xor", $2) << '\t' << reg2str($1->resReg()) << ", " << reg2str($1->resReg())) 
    }
    | X64_SUB real_simd_type X64_REG_2 X64_REG_3 /* mov r2, r1; sub r3, r1 */
    {
        EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic("sub", $2) << '\t' << reg2str($4) << ", " << reg2str($1->resReg())) 
    }
...
real_simd_un_minus
    : X64_UN_MINUS real_simd_type X64_CONST /* mov -c, r1 */
    { 
          EMIT(mnemonic("mov", $2) << '\t' << un_minus_cst($1, $3, true) << ", " << reg2str($1->resReg())) 
    } 
    | X64_UN_MINUS real_simd_type X64_REG_1 /* xor neg_mask, r1 */
    { 
          EMIT(mnemonic("xor", $2) << '\t' << neg_mask($2, true) << ", " << reg2str($1->resReg())) 
    }
    | X64_UN_MINUS real_simd_type X64_REG_2 X64_CONST /* mov r2, r1; xor neg_mask, r1 */ 
    { 
          EMIT(mnemonic("mov", $2) << '\t' <<     reg2str($3) << ", " << reg2str($1->resReg()))
          EMIT(mnemonic("xor", $2) << '\t' << neg_mask($2, true) << ", " << reg2str($1->resReg())) 
    }
    ...
    */
real_simd_div
    : X64_DIV real_simd_type X64_CONST X64_CONST /* mov (c1 / c2), r1 */
    {
        EMIT(mnemonic("mov", $2) << '\t' << cst_op_cst($1, $3, $4, true) << ", " << reg2str($1->resReg())) 
    }
    | X64_DIV real_simd_type X64_CONST X64_REG_2 /* mov c, r1; div r2, r1 */ 
    {
        EMIT(mnemonic("mov", $2) << '\t' << mcst2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic("div", $2) << '\t' <<  reg2str($4) << ", " << reg2str($1->resReg())) 
    }
    | X64_DIV real_simd_type X64_REG_1 X64_CONST /* div c, r1 */
    {
        EMIT(mnemonic("div", $2) << '\t' << mcst2str($4) << ", " << reg2str($3)) 
    }
    | X64_DIV real_simd_type X64_REG_1 X64_REG_1 /* load with 1 */
    {
        swiftAssert(false, "TODO");
    }
    | X64_DIV real_simd_type X64_REG_1 X64_REG_2 /* div r2, r1 */
    {
        EMIT(mnemonic("div", $2) << '\t' << reg2str($4) << ", " << reg2str($3)) 
    }
    | X64_DIV real_simd_type X64_REG_2 X64_CONST /* mov r2, r1; div c, r1 */ 
    {
        EMIT(mnemonic("mov", $2) << '\t' <<  reg2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic("div", $2) << '\t' << mcst2str($4) << ", " << reg2str($1->resReg())) 
    }
    | X64_DIV real_simd_type X64_REG_2 X64_REG_2 /* load with 1 */
    {
        swiftAssert(false, "TODO");
    }
    | X64_DIV real_simd_type X64_REG_2 X64_REG_3 /* mov r2, r1; div r3, r1 */
    {
        EMIT(mnemonic("mov", $2) << '\t' << reg2str($3) << ", " << reg2str($1->resReg()))
        EMIT(mnemonic("div", $2) << '\t' << reg2str($4) << ", " << reg2str($1->resReg())) 
    }
...
real_simd_cmp
    : cmp real_simd_type X64_CONST X64_CONST /* mov true, r1 or mov flase, r1 */
    { 
        EMIT("movb\t" << cst_op_cst($1, $3, $4, true) << ", " << reg2str($1->resReg())) 
    }
    | cmp real_simd_type any_reg X64_CONST /* cmp c, r */
    { 
        EMIT(mnemonic("ucomi", $2) << '\t' << mcst2str($4) << ", " << reg2str($3))
        if ($1->resReg()->color_ != me::Var::DONT_COLOR)
            EMIT("set" << ccsuffix($1, $2) << "b\t" << reg2str($1->resReg())) 
    }
    | cmp real_simd_type X64_CONST any_reg /* cmpn c, r */
    { 
        EMIT(mnemonic("ucomi", $2) << '\t' << mcst2str($3) << ", " << reg2str($4))
        if ($1->resReg()->color_ != me::Var::DONT_COLOR)
            EMIT("set" << ccsuffix($1, $2, true) << "b\t" << reg2str($1->resReg())) 
    }
    | cmp real_simd_type any_reg any_reg   /* cmp r, r */
    { 
        EMIT(mnemonic("ucomi", $2) << '\t' << reg2str($4) << ", " << reg2str($3))
        if ($1->resReg()->color_ != me::Var::DONT_COLOR)
            EMIT("set" << ccsuffix($1, $2) << "b\t" << reg2str($1->resReg())) 
    }
    ;
