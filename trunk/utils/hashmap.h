...
/**
 * This hashCode template used by HashFuc
 ...
 */
template<class T>
inline uint32_t hashCode(const T& t)
{
...
template<>
inline uint32_t hashCode(const uint64_t& ui64)
{
    // just add the lower to the higer 32 bit
    // perhaps there is a smarter way to do this
    uint64_t high = ui64 >> 32;
    uint64_t low  = ui64 << 32;
    low >>= 32;

    return static_cast<uint32_t>(low + high);
}

/// @brief specialization for int64_t
template<>
inline uint32_t hashCode(const int64_t& i64)
{
...
template<>
inline uint32_t hashCode(const float& f)
{
...
template<>
inline uint32_t hashCode(const double& d)
{
 ...
 * This is the same formula as used in Java for String.
 */
template<>
